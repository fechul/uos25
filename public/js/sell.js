var sell = {
        ...
        $('#sell_by_cash').click(function() {
            if ($('#sell_price_sum').val() == 0) {
                ...
                for (var i = 0; i < self.table_data.length; i++) {
                    if (self.table_data[i].EVENT_CD) {
                        var event_info = self.events[self.table_data[i].EVENT_CD];
                        if (event_info.MIN_CNT != event_info.REQ_CNT) {
                            // n + n
                            if (self.table_data[i].REG_PRICE !== self.table_data[i].SELL_PRICE) {
                                LIST.push({
                                    PRDT_CD: self.table_data[i].PRDT_CD,
                                    PRDT_CNT: parseInt(self.table_data[i].SELL_PRICE / self.table_data[i].PRDT_PRICE, 10),
                                    EVENT_APPLY: 'n',
                                    REG_PRICE: self.table_data[i].SELL_PRICE,
                                    SELL_PRICE: self.table_data[i].SELL_PRICE
                                });
                                LIST.push({
                                    PRDT_CD: self.table_data[i].PRDT_CD,
                                    PRDT_CNT: parseInt(self.table_data[i].DSC_PRICE / self.table_data[i].PRDT_PRICE, 10),
                                    EVENT_APPLY: 'y',
                                    EVENT_CD: self.table_data[i].EVENT_CD,
                                    REG_PRICE: self.table_data[i].DSC_PRICE,
                                    SELL_PRICE: 0
                                });
                            } else {
                                LIST.push({
                                    PRDT_CD: self.table_data[i].PRDT_CD,
                                    PRDT_CNT: self.table_data[i].PRDT_CNT,
                                    EVENT_APPLY: 'n',
                                    REG_PRICE: self.table_data[i].REG_PRICE,
                                    SELL_PRICE: self.table_data[i].SELL_PRICE
                                });
                            }
                        } else {
                            if (event_info.GIFT_CD) {
                                LIST.push({
                                    PRDT_CD: self.table_data[i].PRDT_CD,
                                    PRDT_CNT: self.table_data[i].PRDT_CNT,
                                    EVENT_APPLY: 'n',
                    ...
                    } else if (self.table_data[i].IS_GIFT) {
                        // 증정품
                        if (self.table_data[self.find_data_idx(self.gifts[self.table_data[i].PRDT_CD].PRDT_CD)].PRDT_CNT < self.table_data[i].PRDT_CNT) {
                            LIST.push({
                                PRDT_CD: self.table_data[i].PRDT_CD,
                                PRDT_CNT: parseInt(self.table_data[i].SELL_PRICE / self.table_data[i].PRDT_PRICE, 10),
                                EVENT_APPLY: 'n',
                            ...
                            console.log(self.table_data[self.find_data_idx(self.gifts[self.table_data[i].PRDT_CD].PRDT_CD)]);
                            LIST.push({
                                PRDT_CD: self.table_data[i].PRDT_CD,
                                PRDT_CNT: parseInt(self.table_data[i].DSC_PRICE / self.table_data[i].PRDT_PRICE, 10),
                                EVENT_APPLY: 'y',
                                EVENT_CD: self.table_data[self.find_data_idx(self.gifts[self.table_data[i].PRDT_CD].PRDT_CD)].EVENT_CD,
                                REG_PRICE: self.table_data[i].DSC_PRICE,
                                SELL_PRICE: 0
                            });
                        } else {
            ...
            if ((product_data.EVENT_CHECK == 'y') && product_data.EVENT_CD) {
                ...
                if (product_event.MIN_CNT == product_event.REQ_CNT) {
                    if (product_event.GIFT_CD) {
                        ...
                        if (gift_idx >= 0) {
                            self.table_data[gift_idx].IS_GIFT = true;
                ...
                if (self.gifts[PRDT_CD]) {
                    self.table_data[idx].IS_GIFT = true;
    ...
    draw_table: function() {
        ...
        this.table.rows.add(this.table_data);
    ...
    init_table: function() {
        ...
        this.table = $('#sell_list').DataTable({
            'columns': [
                {'data': 'PRDT_CD', 'title': '상품코드', 'width': '11%'},
                {'data': 'PRDT_NAME', 'title': '상품명', 'width': '20%'},
                {'data': 'EVENT_NAME', 'title': '이벤트', 'width': '13%'},
                {'data': 'PRDT_CNT', 'title': '수량', 'width': '8%'},
                {'data': 'STOCK_CNT', 'title': '재고', 'width': '8%'},
                {'data': 'PRDT_PRICE', 'title': '단가', 'width': '10%'},
                {'data': 'REG_PRICE', 'title': '정가', 'width': '10%'},
                {'data': 'DSC_PRICE', 'title': '할인', 'width': '10%'},
                {'data': 'SELL_PRICE', 'title': '판매가', 'width': '10%'},
            ],
            'columnDefs': [
                {
                'targets': 3,
